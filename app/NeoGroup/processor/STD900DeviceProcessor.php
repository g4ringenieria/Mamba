...
use NeoGroup\model\Event;
use NeoGroup\model\Report;

class STD900DeviceProcessor extends DeviceProcessor
{
    protected function checkDatagramValidity ($datagram)
    {
        $datagram = preg_replace( '/[^[:print:]]/', '',$datagram);
        return (substr($datagram, 0, 3) == "<ID" && substr($datagram, -1, 1) == ">") ? true : false;
    }
    
    protected function datagramReceived (Connection $connection, $dataReceived)
    {
        $dataReceived = preg_replace( '/[^[:print:]]/', '',$dataReceived);
        
        // String de conexión: <ID022432>
        if (substr($dataReceived, 0, 3) == "<ID" && substr($dataReceived, -1, 1) == ">") {
            ...
            $this->getLogger()->debug ("Datagram: " . $dataReceived);
            $connection->send(">AK<");
        }
        // String de datos: >REV021586350852+0000000+0000000000000090;ID=022432;*04<
        else if (substr($dataReceived, 0, 1) == ">" && substr($dataReceived, -1, 1) == "<") {
            $deviceId = intval(substr($dataReceived,strpos($dataReceived, ";ID=") + 4, 6));
            $this->getLogger()->debug ("Datagram: " . $dataReceived);
            $connection->send(">AK<");
            
//            $navParam['eventoid'] = $this->eventosMapper($parsedSaip[2]);
//            $navParam['fechaposicion'] = $this->datetimeFromSAIP($parsedSaip[5], $parsedSaip[3], $parsedSaip[4]);
//            $navParam['latitud'] = $this->coordSAIP($parsedSaip[6], "lat");
//            $navParam['longitud'] = $this->coordSAIP($parsedSaip[7], "lon");
//            $navParam['velocidad'] = miles2kms((strlen($parsedSaip[8]) == 5) ? ($parsedSaip[8] / 100) : $parsedSaip[8]);
//            $navParam['rumbo'] = $parsedSaip[9];
//            $navParam['validez'] = $parsedSaip[10] . $parsedSaip[11];
